    ...
    <div v-cloak class="flex min-h-screen">
        ...
        <div class="content">
            <div class="flex items-center relative shadow h-header bg-white z-20 px-6">
                ...
                <div class="relative z-50 w-full max-w-xs px-6">

                    <form class="relative" method="POST" action="{{ route('nova.set_project') }}">

                        @csrf

                        <select class="w-full form-control form-select" name="project_id" onchange="this.form.submit()">
                            <option value="" {{ is_null(session('project_id')) ? 'selected' : '' }}>Seleccionar Proyecto</option>

                            @foreach (Auth::user()->getAllPermissions() as $permission)
                                <option value="{{ $permission->id }}" {{ $permission->id == session('project_id') ? 'selected' : '' }}>{{ $permission->name }}</option>
                            @endforeach
